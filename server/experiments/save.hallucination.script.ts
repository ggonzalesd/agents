...
const map = {
	game_gpt_5_mini: 'hallucination_gpt-5-mini_',
	game_gpt_4_1: 'hallucination_gpt-4.1_',
	game_gpt_4_1_mini: 'hallucination_gpt-4.1-mini_',
	game_gpt_4_1_nano: 'hallucination_gpt-4.1-nano_',
...
async function saveExperimentScript() {
	...
	const file = fs.globSync(
		`server/experiments/results/hallucination/${filenameStart}*.jsonl`,
	)[0];
	...
	for (const line of lines) {
		...
		await prisma.experimentHallucinationResults.updateMany({
			where: {
				id: uuid,
			},
			data: {
				llmHallucinationScore: Number(value),
			},
		});
	}
}
