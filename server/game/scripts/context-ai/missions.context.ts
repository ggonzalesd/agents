	...
	toStringContext(): string {
			...
			const createdLines = this.missions.created.map((m) => {
				const acceptances =
					'acceptances' in m
						...
						: 0;
				return `- [${m.id.slice(0, 8)}] "${m.title}" (${m.status}, ${acceptances} accepted)${m.reward ? ` | Reward: ${m.reward}` : ''}`;
			});
			sections.push(
		...
		if (this.missions.accepted.length > 0) {
			const acceptedLines = this.missions.accepted.map((m) => {
				...
				return `- [${m.id.slice(0, 8)}] "${m.title}" by ${m.creatorType}:${m.creatorId.slice(0, 8)} (Your status: ${acceptance?.status ?? 'unknown'})${m.reward ? ` | Reward: ${m.reward}` : ''}`;
			});
			sections.push(
		...
		if (this.missions.available.length > 0) {
			const availableLines = this.missions.available.map(
				(m) =>
					`- [${m.id.slice(0, 8)}] "${m.title}" by ${m.creatorType}:${m.creatorId.slice(0, 8)}${m.reward ? ` | Reward: ${m.reward}` : ''}`,
			);
			sections.push(
