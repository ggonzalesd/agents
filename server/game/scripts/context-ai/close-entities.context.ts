		...
		const closeEntities = this.world
			...
			.filter(({ entity }) => {
				const stats = entity
					...
					.raw();
				if (stats?.type == null && stats?.kind == null) return true;
				const blockedTypes = ['rock', 'floating_text', 'trigger_zone'];
				if (stats?.type && blockedTypes.includes(stats.type)) return false;
				return true;
			})
		.map((other) => {
