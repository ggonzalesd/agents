...
const EXPERIMENT_KEY = 'NPCS-CON-LLMS';
...
import { RequestGatherItemsPhaseEcs } from '../phases/request-gather-items.phase.ecs';
...
const phaseFactories: {
	...
	'request-gather-items': (def, runtime) => new RequestGatherItemsPhaseEcs(def, runtime),
};
...
export class NpcsConLlmsExperimentRuntimeEcs extends ExperimentRuntimeEcs {
	...
	protected async onExperimentMount(): Promise<void> {
		...
		console.log(
			'[NpcsConLlms] OnExperimentMount: Allocated slot for user:',
			this.userId,
			'at position:',
			position,
		);
		...
		this.mapId = `experiment-npcs-con-llms-${this.userId}`;
		...
		console.log(
			'[NpcsConLlms] OnExperimentMount: Setup complete for user:',
			this.userId,
		);
	}
